...
* **Suspicious Outbound Connections:** مراقبة أي اتصال خارجي غير معتاد صادر من أجهزة الموظفين باتجاه خدمات تخزين سحابية (مثل OneDrive) متبوعاً بتحميل ملفات تنفيذية.
* **Unusual Ports (Non-Standard):** تفعيل تنبيهات عند رصد تواصل عبر المنفذ `9138` أو أي منافذ غير قياسية للـ HTTP/TCP.
* **Encrypted Beaconing Patterns:** البحث عن اتصالات دورية (Periodic communication) ذات حجم بيانات ثابت ومتكرر، مما يشير إلى وجود برمجية تحكم عن بعد (RAT).
* **Registry Persistence:** فحص سجلات النظام (Registry) لمراقبة أي تغييرات في مفاتيح التشغيل التلقائي (Auto-run keys) التي تستخدمها برمجية Remcos لضمان بقائها.
* **Dynamic DNS Usage:** مراقبة استعلامات الـ DNS التي تستخدم خدمات الـ Dynamic DNS أو خدمات الاستعلام الجغرافي (مثل `geoplugin`) بشكل مفاجئ.
## دروس مستفادة وتحديات (Personal Reflection)
* **تحدي واجهته:** في البداية، كان من الصعب تمييز حركة الـ C2 عن حركة الشبكة العادية، لكن باستخدام فلاتر Wireshark المتقدمة (مثل تحليل الـ SYN Packets والـ Beaconing frequency)، استطعت حصر السيرفر المشبوه.
